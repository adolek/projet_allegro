...
void niveau3()
{
    ...
    decor=load_bitmap_check("Map_niveau3.bmp");    // chargement du decor
    ...
    while (!key[KEY_ESC])
    {
        ...
        if(NiveauSuivant<=50)
        {
            textprintf_ex(decor, font, 250,50, makecol(255,255,255), -1, "WARNING ! AVOID THE LAVA OR YOUR WILL BURN");

        }
        if(NiveauSuivant>=3500)
        {
            textprintf_ex(screen, font, 275,300, makecol(255,255,255), -1, "CONGRATULATIONS ! LEVEL 3 SUCCEEDED");
            rest(4000);
            boss();
        }
